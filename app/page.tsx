...
const BASE_URL = "https://yourapp.vercel.app";   // replace after deploy

// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// GIFT CATALOG — 25 curated ideas with affiliate placeholder links
// Set affiliate: null until you have real links, then paste them in
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
const GIFT_CATALOG = [
  // ── Beauty & Wellness ──────────────────────────────────────────
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// SIMULATED ACCESS CODE STORE (replace with Supabase later)
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
const generateCode = () => Math.random().toString(36).substring(2, 8).toUpperCase();

// In-memory store: { code: { used: false, roulette: null } }
const codeStore = {};
...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .card-accent {
    ...
    background: linear-gradient(135deg, var(--card) 0%, #1a1710 100%);
  }
  ...
  .chamber-outer {
    ...
    background: radial-gradient(circle at 38% 32%, #1c1a14, #0f0f0f);
    display: flex;
    align-items: center;
    justify-content: center;
    position: relative;
    margin: 0 auto 1rem;
    box-shadow: 0 0 50px rgba(212,168,67,0.07), inset 0 0 40px rgba(0,0,0,0.6);
  }
  ...
  .verdict-desc { font-size: 11px; color: var(--text-3); margin-bottom: 0.85rem; }
    ...
    <div className="gift-slot">
      ...
      {mode === "catalog" ? (
        ...
        <div className="gap-sm">
          <input
            className="input"
            placeholder="Gift title (e.g. Thai massage voucher)"
            value={customTitle}
            onChange={e => setCustomTitle(e.target.value)}
          />
          <input
            className="input"
            placeholder="Link (optional — product, booking, etc.)"
            value={customLink}
...
export default function App() {
  // screen: "entry" | "owner" | "sender" | "preview" | "link-ready" | "receiver"
  const [screen, setScreen] = useState("entry");
  const [pwInput, setPwInput] = useState("");
  const [codeInput, setCodeInput] = useState("");
  const [pwError, setPwError] = useState(false);
  const [codeError, setCodeError] = useState(false);
  const [entryMode, setEntryMode] = useState("code"); // "owner" | "code"
  ...
  // Receiver state (loaded from "link")
  ...
  useEffect(() => {
    // Check if URL has a roulette code (simulated for demo)
    const params = new URLSearchParams(window.location.search);
    const r = params.get("r");
    if (r && codeStore[r]?.roulette) {
      const data = codeStore[r].roulette;
      setReceiverGifts(data.gifts);
      setReceiverVenmo(data.venmo);
      setReceiverSenderName(data.senderName);
      setScreen("receiver");
    }
  }, []);
  ...
  const tryCode = () => {
    ...
    if (codeStore[c] && !codeStore[c].used) {
      setActiveCode(c);
      setScreen("sender");
    } else {
      setCodeError(true);
  ...
  // ── OWNER: generate codes ──────────────────────────────────────
  const makeCode = () => {
    const c = generateCode();
    codeStore[c] = { used: false, roulette: null };
  ...
  // ── GENERATE LINK ──────────────────────────────────────────────
  const generateLink = () => {
    // Save roulette data to code slot
    codeStore[activeCode] = {
      used: true,
      roulette: {
        gifts: filledGifts,
        venmo: senderVenmo,
  ...
  const senderVenmoLink = receiverVenmo
    ? `https://venmo.com/${receiverVenmo}?txn=pay&amount=50&note=Accountability+Tribute+💸`
    : null;
        ...
        <div className="wrap">
          ...
          <div className={`card card-accent ${pwError || codeError ? "shake" : ""}`}>
            ...
            <div className="row mb1">
              <button
                className={`btn ${entryMode === "code" ? "btn-gold" : "btn-ghost"}`}
                onClick={() => setEntryMode("code")}
              >Sender Access</button>
              <button
                className={`btn ${entryMode === "owner" ? "btn-gold" : "btn-ghost"}`}
                onClick={() => setEntryMode("owner")}
              >Owner</button>
            </div>
            ...
            {entryMode === "code" ? (
              <div className="gap-sm">
                ...
                <input
                  ...
                  style={{ letterSpacing: "0.22em", textAlign: "center", fontSize: "1.1rem" }}
                />
              ...
              <div className="gap-sm">
                ...
                <input
                  ...
                  onKeyDown={e => e.key === "Enter" && tryOwner()}
      ...
      <div className="app">
        <div className="wrap">
          ...
          <div className="card card-accent mb1">
            ...
            <p className="sub">Generate a code, sell it, and the buyer gets one-time sender access. Each code can only be used once.</p>
            ...
            <div className="card fadein">
              ...
              {generatedCodes.map(c => (
                <div className="code-row" key={c}>
                  ...
                  <span className={`code-status ${codeStore[c]?.used ? "used" : ""}`}>
                    {codeStore[c]?.used ? "Used" : "Available"}
                  </span>
                  <button
          ...
          <div className="card">
            ...
            <p className="sub">Buyers pay you before you share the code. Direct them here:</p>
            <div className="center mt1">
              <span className="code-pill">@{OWNER_VENMO}</span>
            </div>
            <a href={ownerVenmoLink} className="btn btn-venmo mt2" target="_blank" rel="noreferrer">
        ...
        <div className="wrap">
          ...
          <div className="card card-accent">
            ...
            <div className="gap-sm">
              ...
              />
              <label className="label mt1">Your Venmo handle (for receiver's fallback payment)</label>
              <input
                className="input"
                placeholder="@your-venmo"
                value={senderVenmo}
                onChange={e => setSenderVenmo(e.target.value)}
              />
            </div>
          </div>
          ...
          <div className="card">
            ...
            <p className="sub mb1">Choose 10 gifts. Pick from the catalog or write your own using "Other" at the bottom of any dropdown.</p>
            <div className="gap">
              {Array.from({ length: 10 }).map((_, i) => (
                <GiftSlot
                  ...
                  index={i}
                  value={gifts[i]}
                  onChange={val => handleGiftChange(i, val)}
                  usedIds={usedIds}
                />
              ))}
            </div>
          ...
          <div className="gap">
            <button
              ...
              onClick={goPreview}
              disabled={filledGifts.length < 10 || !senderName.trim()}
            >
              Preview My Roulette →
            </button>
            <p className="sub center">
              {filledGifts.length}/10 gifts selected
              {!senderName.trim() ? " · Add your name above" : ""}
            </p>
          </div>
      ...
      <div className="app">
        <div className="wrap fadein">
          ...
          <div className="card">
            ...
            {filledGifts.map((g, i) => (
              <div className="code-row" key={i}>
                ...
                {g.affiliate && <span style={{ fontSize: "10px", color: "var(--gold)", letterSpacing: "0.1em" }}>LINKED</span>}
              </div>
            ))}
          </div>
          ...
          <div className="tip-box fadein">
            ...
            <p className="sub mb1">
              Enjoying the chamber? Send a small tip to the person who built this — keeps the operation running.
            </p>
            <p style={{ fontFamily: "var(--serif)", fontSize: "1.75rem", color: "var(--gold)", marginBottom: "0.25rem" }}>
              ${OWNER_VENMO_AMOUNT}
            </p>
            <p className="label" style={{ marginBottom: "0.85rem" }}>suggested tip</p>
            <a href={ownerVenmoLink} className="btn btn-venmo" target="_blank" rel="noreferrer" style={{ display: "inline-block", width: "auto", padding: "0.6rem 1.5rem", borderRadius: "8px", fontSize: "12px", letterSpacing: "0.12em", textTransform: "uppercase", textDecoration: "none" }}>
              Tip via Venmo
            </a>
          </div>

          <div className="gap mt2">
            <button className="btn btn-red" onClick={generateLink}>
              Generate My Link →
            </button>
            <button className="btn btn-ghost" onClick={() => setScreen("sender")}>
              ← Edit Gifts
            </button>
          </div>

          <p className="foot">Tip is optional but appreciated. Your link generates either way.</p>
        </div>
      </div>
    </>
  );
      ...
      <div className="app">
        <div className="wrap fadein">
          ...
          <div className="card center">
            ...
            <p className="sub">
              They open the link → no password, no setup → straight into the chamber → 3 spins → they get their consequence. If they refuse, they hit your Venmo instead.
            </p>
          </div>
        ...
        <div className="wrap">
          ...
          {receiverSenderName && (
            <div className="card center mb1 fadein">
              <p className="sub" style={{ fontSize: "11px" }}>
                A consequence has been prepared for you by
              </p>
              <p style={{ fontFamily: "var(--serif)", fontSize: "1.3rem", color: "var(--gold)", marginTop: "0.25rem" }}>
                {receiverSenderName}
              </p>
            </div>
          )}
          ...
          <div className="card card-accent">
            ...
            <div className="dots" style={{ marginBottom: "1.25rem" }}>
              {[0,1,2].map(i => (
                <div key={i} className={`dot ${i < spinsLeft ? "on" : ""}`} />
              ))}
            </div>
            ...
            {selectedGift && verdict && !isSpinning && (
              <div className="verdict pulsein" key={resultKey}>
                ...
                {selectedGift.affiliate && (
                  <a href={selectedGift.affiliate} target="_blank" rel="noreferrer" className="verdict-link">
                    View / Shop ↗
                  </a>
                )}
              </div>
            ...
            <div className="gap">
              ...
              {(spinsLeft <= 0 || selectedGift) && !isSpinning && senderVenmoLink && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  style={{ display: "block", textAlign: "center", textDecoration: "none", padding: "0.85rem", fontSize: "12px", letterSpacing: "0.12em", textTransform: "uppercase" }}
                >
          ...
          <p className="foot">
            Consequences are non-negotiable. Refusal results in monetary tribute.
            <br />Powered by {APP_TITLE}.
          </p>
        </div>
      </div>
    </>
